...
router.get('/', requireAuth, (req, res) => {
  School.find({ user: req.user.id }, { __v: 0, user: 0 }, (err, school) => {
    if (err) {
      res.status(400).send({ message: 'Get schools failed', err });
    } else {
      res.send({ message: 'schools retrieved successfully', school });
    }
  });
});
...
// router.get('/', requireAuth, (req, res) => {
//   const school = School || {};
//   res.send({ message: 'School info successfully retreived', school });
// });
